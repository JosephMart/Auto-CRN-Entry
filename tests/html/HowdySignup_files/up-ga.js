...
(function($, _) {
   ...
   var getEventLinkFname = function(clickedLink) {
   ...
   var addFlyoutHandlers = function() {
      $("ul.fl-tabs li.portal-navigation a.portal-subnav-link").click(function(event) {
         ...
         var portletFlyoutTab = clickedLink.parents("li.portal-navigation").children("a.portal-navigation-link").text();
         var portletFlyoutTitle = clickedLink.find("span.portal-subnav-label").text();
         var portletFlyoutLabel = portletFlyoutTab + " > " + portletFlyoutTitle;

         // Find the target portlet's fname
         var fname = clickedLink.prop("href");
         ...
         handleLinkClickEvent(event, clickedLink, {
            ...
            'eventAction' : fname,
            'eventLabel' : portletFlyoutLabel
         });
      });
   };

   /**
    * TAMU Custom function starts
    */

   /**
    * Inspects all clicks on links, any of the clicks that result in existing to a different host are tracked as Outbound Link events
    *
    */
   var addHowdySearchHandlers = function() {
      $( "form#gsearch_tamu" ).submit(function( event ) {
         var clickedForm = $(this);
         // Setup the page level state
         setPageVariables();

         var searchtext = clickedForm.find("input[name='q']").val();
         handleLinkClickEvent(event, clickedForm, {
            'eventCategory' : 'Search',
            'eventAction' : "Google Campus Search > CS",
            'eventLabel' : searchtext
         });
      });

      $( "form#gsearch_tamug" ).submit(function( event ) {
         var clickedForm = $(this);
         // Setup the page level state
         setPageVariables();
         ...
         handleLinkClickEvent(event, clickedForm, {
            'eventCategory' : 'Search',
            'eventAction' : "Google Campus Search > GV",
            'eventLabel' : searchtext
         });
      });
   ...
   var addHowdyLinkHandlers = function() {
      $('a').click(function(event) {
         var clickedLink = $(this);
         var clinkedLinkText;


         if(clickedLink.hasClass('result_link')) {  // For Howdy Search Result click
            setPageVariables();

            clinkedLinkText = clickedLink.find("span.result_title").text();
            handleLinkClickEvent(event, clickedLink, {
               'eventCategory' : 'Search',
               'eventAction' : "Howdy Search > result click",
               'eventLabel' : clinkedLinkText
            });
         }
         else
         if(clickedLink.parent().is("div.quickservicelink-icon") || clickedLink.parent().parent().is("div.quickservicelink-icon")){
            // Setup the page level state
            setPageVariables();

            clinkedLinkText = clickedLink.closest("div.quickservicelink-icon").find("a").attr("title");
            handleLinkClickEvent(event, clickedLink, {
               'eventCategory' : 'QuickLink Icons',
            ...
            var clickedLinkTab;
            var clickedLinkChannel = "";
            if (typeof up.analytics.pageData.tab != 'undefined') {
               clickedLinkTab = up.analytics.pageData.tab.tabName;
               clickedLinkChannel = clickedLink.closest("div.up-portlet-wrapper-inner").find("div.up-portlet-titlebar h2 > a").text();
            }
            else if(clickedLink.children().first().is("#howdy-logo") || clickedLink.children().first().is("#sticky-header-img-howdy")) {
               clickedLinkTab = "Howdy Logo";
            }
            else if(up.analytics.userstatus == 'undefined' || up.analytics.userstatus == 'guest') {
               clickedLinkTab = "Landing Page";
            }

            if (clickedLink.text() != null || clickedLink.text() != "") {
               clinkedLinkText = clickedLink.text();
            } else if(clickedLink.attr("title") != null || clickedLink.attr("title") != "") {
               clinkedLinkText = clickedLink.attr("title");
            }


            // GA Event creation logic starts
            if(clickedLinkTab == "Landing Page") {
               // Send the event and deal with the click
               handleLinkClickEvent(event, clickedLink, {
               ...
               if (linkHost != "" && linkHost != document.domain) {
                  ...
                  handleLinkClickEvent(event, clickedLink, {
                     ...
                     'eventAction' : clickedLink.prop("href"),
                     'eventLabel' : clickedLinkTab + " > " + clickedLinkChannel + " > " + clinkedLinkText
                  });
               }
               else
               {
                  var clickedLinkURL = clickedLink.attr("href");
                  if(clickedLinkURL.includes("TAMU-APP-Launcher")) {
                     var linkSSB = clickedLink.prop("pP_targetEndpoint");
                     if (linkSSB != "") {
                        setPageVariables();

                        // Send the event and deal with the click
                        handleLinkClickEvent(event, clickedLink, {
                           'eventCategory': 'SSB Link',
                           'eventAction': clickedLink.prop("href"),
                           'eventLabel': clickedLinkTab + " > " + clickedLinkChannel + " > " + clinkedLinkText
                        });
                     }
                  }
                  else if(clickedLinkURL.includes("TAMU-XE-Launcher")) {
                     var linkSSB = clickedLink.prop("pP_targetEndpoint");
                     if (linkSSB != "") {
                        setPageVariables();
                        ...
                        // Send the event and deal with the click
                        handleLinkClickEvent(event, clickedLink, {
                           'eventCategory': 'SSB Link - Banner9',
                           'eventAction': clickedLink.prop("href"),
                           'eventLabel': clickedLinkTab + " > " + clickedLinkChannel + " > " + clinkedLinkText
                        });
                     }
                  }
                  else if(clickedLinkURL.includes("Inside")) {
                     // Send the event and deal with the click
                     handleLinkClickEvent(event, clickedLink, {
                        'eventCategory': 'Inside Link',
                        'eventAction': clickedLink.prop("href"),
                        'eventLabel': clickedLinkTab + " > " + clickedLinkChannel + " > " + clinkedLinkText
                     });
                  }
                  else if(clickedLinkURL.includes("mailto")) {
                     // Send the event and deal with the click
                     handleLinkClickEvent(event, clickedLink, {
                        'eventCategory': 'Email',
                        'eventAction': clinkedLinkText,
                        'eventLabel': clickedLinkTab + " > " + clickedLinkChannel + " > " + clinkedLinkText
                     });
                  }
               }
   ...
   $(document).ready(function() {
      ...
      addHowdyLinkHandlers();
      addHowdySearchHandlers();
